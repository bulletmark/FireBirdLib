...
type
  ...
  TForm1 = class(TForm)
    ...
    Button_FontPath1    : TButton;
    Button_FontPath2    : TButton;
    Label_FontPath1     : TLabel;
    Label_FontPath2     : TLabel;
    OpenDialog1         : TOpenDialog;
    Button_Combine      : TButton;

    procedure Button_FontPath1Click(Sender: TObject);
    procedure Button_FontPath2Click(Sender: TObject);
    procedure CheckListBox1ClickCheck(Sender: TObject);
    procedure CheckListBox2ClickCheck(Sender: TObject);

    procedure Button_CombineClick(Sender: TObject);

  private
    var
      FontDef1, FontDef2     : array of tFontDefUC;
      NrEntries1, NrEntries2 : integer;
...
procedure TForm1.Button_FontPath1Click(Sender: TObject);

var
  f                     : file;
  Hdr                   : array[0..4] of AnsiChar;
  i, j, t               : integer;
  pFontDefUC            : ^tFontDefUC;
...
begin
  ...
  begin
    DefaultExt := 'ufnt';
    Title := 'FontCombine: select Unicode font 1';
    Filter := 'FBLib ufnt (*.ufnt)|*.ufnt|All files|*.*';
    FileName := '';
    Execute(self.WindowHandle);
    If FileName = '' Then exit;

    Label_FontPath1.Caption := FileName;
    CheckListBox1.Clear();

    ZeroMemory(@UnicodeHits[0], sizeof(UnicodeHits));
    ...
    BlockRead(f, NrEntries1, sizeof(dword));
    SetLength(FontDef1, NrEntries1);
    BlockRead(f, FontDef1[0], NrEntries1 * sizeof(tFontDefUC));
    CloseFile(f);

    pFontDefUC := @FontDef1[0];
    for i := 0 to NrEntries1 - 1 do
    begin
      t := UnicodeToTable(pFontDefUC.Unicode);
    ...
    begin
      s := Format('%s (0x%6.6x)', [UnicodeTable[i].TableName, UnicodeTable[i].UnicodeStart]);
      j := CheckListBox1.Items.Add(s);

      //Gibt es den Eintrag in der anderen Liste?
      if CheckListBox2.Items.IndexOf(s) = -1 then
        CheckListBox1.Checked[j] := true;
    end;
  end;
end;

procedure TForm1.Button_FontPath2Click(Sender: TObject);
...
begin
  ...
  begin
    DefaultExt := 'ufnt';
    Title := 'FontCombine: select Unicode font 2';
    Filter := 'FBLib ufnt (*.ufnt)|*.ufnt|All files|*.*';
    FileName := '';
    Execute(self.WindowHandle);
    If FileName = '' Then exit;

    Label_FontPath2.Caption := FileName;
    CheckListBox2.Clear();
    ...
    BlockRead(f, NrEntries2, sizeof(dword));
    SetLength(FontDef2, NrEntries2);
    BlockRead(f, FontDef2[0], NrEntries2 * sizeof(tFontDefUC));
    CloseFile(f);

    pFontDefUC := @FontDef2[0];
    for i := 0 to NrEntries2 - 1 do
    begin
      t := UnicodeToTable(pFontDefUC.Unicode);
      if t >= 0 then inc(UnicodeHits[t]);
      inc(pFontDefUC);
    ...
    begin
      s := Format('%s (0x%6.6x)', [UnicodeTable[i].TableName, UnicodeTable[i].UnicodeStart]);
      j := CheckListBox2.Items.Add(Format('%s (0x%6.6x)', [UnicodeTable[i].TableName, UnicodeTable[i].UnicodeStart]));
...
begin
  ...
  begin
    ...
    for i := 0 to CheckListBox2.Count - 1 do
      if CheckListBox2.Items[i] = s then
      begin
...
begin
  ...
  begin
    s := CheckListBox2.Items[CheckListBox2.ItemIndex];
    for i := 0 to CheckListBox1.Count - 1 do
      if CheckListBox1.Items[i] = s then
      begin
...
procedure TForm1.Button_CombineClick(Sender: TObject);
...
var
  ...
  NewFontCharCount      : integer;
  NewFontEntries        : array of tFontDefUC;
  pNewFontEntries       : ^tFontDefUC;
  NewFontBitmapSize     : integer;
  NewFontBitmap         : array of byte;
  pNewFontBitmap        : integer;

begin
  tOld := -1;

  //Benötigten Platz berechnen. Erster Font
  NewFontCharCount := 0;
  NewFontBitmapSize := 0;
  ...
  for i := 0 to NrEntries1 - 1 do
  begin
    t := UnicodeToTable(pFontDef.Unicode);
    if t <> tOld then
    begin
      s := Format('%s (0x%6.6x)', [UnicodeTable[t].TableName, UnicodeTable[t].UnicodeStart]);
      k := CheckListBox1.Items.IndexOf(s);
      c := CheckListBox1.Checked[k];
      tOld := t;
    end;

    //Ist die Tabelle ausgewählt?
    if c then
    begin
      inc(NewFontCharCount);
      inc(NewFontBitmapSize, pFontDef.Width * pFontDef.Height);
    end;
  ...
  begin
    ...
    begin
      s := Format('%s (0x%6.6x)', [UnicodeTable[t].TableName, UnicodeTable[t].UnicodeStart]);
      k := CheckListBox2.Items.IndexOf(s);
      c := CheckListBox2.Checked[k];
      tOld := t;
    end;
  ...
  SetLength(NewFontEntries, NewFontCharCount);
  SetLength(NewFontBitmap, NewFontBitmapSize);

  //Daten umkopieren.
  pNewFontEntries := @NewFontEntries[0];
  pNewFontBitmap := 0;

  //Erster Font
  AssignFile(f, Label_FontPath1.Caption);
  Reset(f, 1);
  tOld := -1;
  ...
  begin
    ...
    if c then
    begin
      if pNewFontBitmap >= NewFontBitmapSize then
      begin
        MessageBox(Self.WindowHandle, 'Buffer overflow in bitmap', 'FontCombine', MB_OK or MB_ICONSTOP);
        halt(1);
      end;

      //Das Verzeichnis
      pNewFontEntries^ := pFontDef^;
      pNewFontEntries.FileOffset := 8 + NewFontCharCount * sizeof(tFontDefUC) + pNewFontBitmap;
      inc(pNewFontEntries);

      //Das Bitmap
      Seek(f, pFontDef.FileOffset);
      BlockRead(f, NewFontBitmap[pNewFontBitmap], pFontDef.Width * pFontDef.Height);
      inc(pNewFontBitmap, pFontDef.Width * pFontDef.Height);
    end;
  ...
  //Zweiter Font
  AssignFile(f, Label_FontPath2.Caption);
  Reset(f, 1);
  tOld := -1;
  ...
  if pNewFontBitmap <> NewFontBitmapSize then
  begin
    MessageBox(Self.WindowHandle, 'The resulting bitmap doesn''t match the expected size.', 'FontCombine', MB_OK or MB_ICONSTOP);
    halt(1);
  end;

  AssignFile(f, 'Result.ufnt');
  Rewrite(f, 1);
  Hdr[0] := 'U';
  Hdr[1] := 'F';
  Hdr[2] := 'N';
  Hdr[3] := 'T';
  BlockWrite(f, Hdr[0], 4);
  BlockWrite(f, NewFontCharCount, sizeof(dword));
  BlockWrite(f, NewFontEntries[0], NewFontCharCount * sizeof(tFontDefUC));
  BlockWrite(f, NewFontBitmap[0], NewFontBitmapSize);
  CloseFile(f);

  SetLength(NewFontBitmap, 0);
end;
